...
workDir = '/home/caiyideng/zjj/code/3/Dimensional_reduction/'
...
def draw_ROC_Curve(output_list, output_result_list, correct_list, name):
    ...
    plt.plot(fpr, tpr, color='red',
             lw=lw, label='all+dr (ROC_AUC = %0.3f)' % AUC)
    plt.plot([0, 1], [0, 1], color='gray', lw=lw, linestyle='--')
    plt.ylabel('True Positive Rate')
    plt.xlabel('False Positive Rate')
    plt.legend(loc="lower right")
    plt.savefig(plotDir + 'ROC-'+ name + '_' + str(AUC) + ".png")
    plt.close()


def draw_PR_Curve(output_list, output_result_list, correct_list, name):
    y_score = np.array(output_list)
    y_true = np.array(correct_list)
    precision, recall, thresholds = precision_recall_curve(y_true, y_score)
    ...
    plt.plot(precision, recall, color='red',
             lw=lw, label='all+dr (PR_AUC = %0.3f)' % pr_auc)
    plt.plot([0, 1], [1, 0], color='gray', lw=lw, linestyle='--')
    plt.ylabel('Precision')
    plt.xlabel('Recall')
    plt.legend(loc="upper right")
    plt.savefig(plotDir + 'PR-' + name + '_' + str(pr_auc) + ".png")
    plt.close()
    return AUC, pr_auc, F1
